...
public abstract class BusinessLogic<T extends Serializable, D extends GenericHibernateDAO, ID extends Serializable> {
  ...
  public Long rowsCount(HashTable<String, Object> dati)
      throws HibernateException, HibernateUtilException {
    D tableDao = null;
  ...
  public ID save(HashTable<String, Object> dati) throws HibernateException,
      IllegalAccessException, IllegalArgumentException,
      InvocationTargetException, NoSuchMethodException, SecurityException,
      HibernateUtilException, NamingException {
  ...
  public ID save(HashTable<String, Object> dati, boolean forceSave)
      ...
      NoSuchMethodException, SecurityException, HibernateUtilException,
      NamingException {
    D tableDao = null;
    T table = null;
    ID id = null;
    ...
    try {
      ...
      if (!dati.containsKey("id")) {
        ...
        if (table == null) {
          ...
          if (id instanceof String) {
            ...
            table.getClass().getMethod("setId", Integer.class).invoke(table,
                id);
          }
        }
      }
      ...
      result = (table == null
          ? null
          : (ID) table.getClass().getMethod("getId").invoke(table));
    } catch (HibernateException e) {
      e.printStackTrace();
  ...
  protected abstract void postSave(HashTable<String, Object> dati, T table)
      throws NamingException, HibernateUtilException, IllegalAccessException,
  ...
  protected abstract List<T> find(D tableDao, HashTable<String, Object> dati,
      List<Order> orders, int page, int pageSize)
      throws HibernateException, HibernateUtilException;

  protected abstract Criteria rowsCount(D tableDao,
      HashTable<String, Object> dati);

  public abstract void delete(ID id) throws Exception;

  protected abstract void addRecord(T dati)
      throws HibernateException, HibernateUtilException;

  public String toJson(Serializable table) throws SecurityException,
      IllegalAccessException, IllegalArgumentException,
      InvocationTargetException, BusinessLogicException {
    return toJson(table, null);
  }

  @SuppressWarnings("unchecked")
  public String toJson(Serializable table, Class classExclude)
    ...
    try {
      ...
      for (int i = 0; i < m.length; i++) {
        if (m[i].getName().startsWith("get")
            ...
            && !m[i].getName().equalsIgnoreCase("getHandler") && !m[i].getName()
                .equalsIgnoreCase("getHibernateLazyInitializer")) {
          if (m[i].getParameterTypes().length == 0) {
            ...
            if (value != null) {
              ...
              if (classExclude == null || !value.getClass().getName()
                  .equals(classExclude.getName())) {
                key = m[i].getName();
                ...
                if (value instanceof String) {
                  ...
                  Object[] objects;
                  int x = 0;
                  objects = ((PersistentSet) value).toArray();
                  // persistentSet = (PersistentSet) value;
                  for (Object object : objects) {
                    if (x > 0) {
                      jsonArray += ",";
                    }
                    jsonArray += toJson(key, object);
                    x++;
                  }
                  jsonArray += "]";
                } else {

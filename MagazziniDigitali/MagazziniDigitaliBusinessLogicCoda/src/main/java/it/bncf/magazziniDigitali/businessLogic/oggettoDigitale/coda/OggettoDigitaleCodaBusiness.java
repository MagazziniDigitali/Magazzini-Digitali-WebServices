...
public class OggettoDigitaleCodaBusiness extends OggettoDigitaleBusiness {
	...
	public Logger log = LogManager.getLogger(OggettoDigitaleCodaBusiness.class);
	...
	/**
	 * Questo metodo viene utilizzato per gestire la coda del materiale da inviare
	 * negli altri Storage
	 * 
	 * @param application
	 */
	public void coda(Logger logCoda, IMDConfiguration<?> configuration, JobExecutionContext context)
	...
	private void writeFileCoda(String data, String id, String containerName, IMDConfiguration<?> configuration)
			throws FileNotFoundException, MDConfigurationException, IOException {
		File coda = null;
					...
					if (!coda.getParentFile().exists()) {
						if (!coda.getParentFile().mkdirs()) {
							throw new IOException("Riscontrato un problema nella creazione della cartella ["
									+ coda.getParentFile().getAbsolutePath() + "]");
						}
					}
	...
	private File genFileCoda(String data, String id, IMDConfiguration<?> configuration, int conta)
		...
		try {
			coda = new File(configuration.getSoftwareConfigString("coda.path") + File.separator + data.substring(0, 4)
					+ File.separator + data.substring(4, 6) + File.separator + data.substring(6, 8) + File.separator
